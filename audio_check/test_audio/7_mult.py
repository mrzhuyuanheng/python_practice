...
def main_page():
    ...
    def create_row(row_number):
        button_a = ui.button(f'burn Test {row_number}', on_click=test_burn)
        button_b = ui.button(f"Audio Test {row_number}", on_click=test_audio)
        progressbar = ui.linear_progress(value=100).props('instant-feedback')
        queue = Manager().Queue()
        timer = ui.timer(0.1, callback=lambda: progressbar.set_value(queue.get() if not queue.empty() else progressbar.value))
        # progressbar.visible = False

        return button_a, button_b, progressbar, queue, timer
    ...
    with ui.grid(columns=3):
        for i in range(8):
            button_a, button_b, progressbar, queue, timer = create_row(i + 1)
            rows.append((button_a, button_b, progressbar, queue, timer))
