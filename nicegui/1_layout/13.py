...
with ui.column().classes('w-8/12 p-4').style('text-align: left;'):
    ...
    label_first_row = ui.label(f'烧写信息: {value_b}/{value_c} 模式：烧写').classes('text-lg mb-0')  # 减小行间距

    # 创建第二行到第五行的文本
    label_second_row = ui.label('当前芯片： CSK5060').classes('text-lg mb-0')  # 减小行间距
    label_third_row = ui.label('文件名字： fw.bin 已授权').classes('text-lg mb-0')  # 减小行间距
    label_fourth_row = ui.label('校验码： 4A33-76449996').classes('text-lg mb-0')  # 减小行间距

    # 创建八行布局，每行包含一个编号、一个居中显示的文本标签和一个长度较长的进度条
    for i in range(1, 9):
        with ui.row().classes('items-center relative'):  # 移除 mb-4，减小行间距
            # 显示编号
            ui.label(f'{i}').classes('text-lg mr-0')
